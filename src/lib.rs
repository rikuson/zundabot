...
pub fn run() -> Result<(), Box<dyn Error>> {
    ...
    let source = gstreamer_app::AppSrc::builder()
        ...
        .build();
    let parser = gst::ElementFactory::make("wavparse")
        .name("parser")
        .build()
    ...
    // Build the pipeline
    pipeline.add_many([source.upcast_ref(), &parser, &sink]).unwrap();
    gst::Element::link_many(&[source.upcast_ref(), &parser, &sink]).unwrap();
